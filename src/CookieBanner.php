...
namespace VkmToolkit\CookieBanner;

use VkmToolkit\CookieBanner\Config\Configuration;
use VkmToolkit\CookieBanner\Consent\ConsentManager;
use VkmToolkit\CookieBanner\Consent\ConsentData;
use VkmToolkit\CookieBanner\Event\EventDispatcher;
use VkmToolkit\CookieBanner\Event\EventInterface;
use VkmToolkit\CookieBanner\Event\BannerEvent;
use VkmToolkit\CookieBanner\Language\TranslationManager;
use VkmToolkit\CookieBanner\Template\TemplateManager;
use VkmToolkit\CookieBanner\Template\TemplateInterface;
use VkmToolkit\CookieBanner\ScriptBlocker\ScriptBlocker;
...
class CookieBanner
{
    ...
    public function renderCss(): string
    {
        ...
        if ($this->inlineAssets) {
            ...
            if ($css) {
                return "<style id=\"vkm-cookie-banner-css\">\n{$css}\n</style>";
            }
            return '';
        }
        ...
        if ($this->assetsUrl) {
            return sprintf(
                '<link rel="stylesheet" href="%s/css/%s.css" id="vkm-cookie-banner-css">',
    ...
    public function renderJs(): string
    {
        ...
        $output = "<script>window.vkmCookieBannerConfig = {$configJson};</script>\n";

        if ($this->inlineAssets) {
            $jsPath = $this->findAssetFile('js/cookiebanner.js');
            if ($jsPath && file_exists($jsPath)) {
                $js = file_get_contents($jsPath);
                $output .= "<script id=\"vkm-cookie-banner-js\">\n{$js}\n</script>";
            }
        } elseif ($this->assetsUrl) {
            $output .= sprintf(
                '<script src="%s/js/cookiebanner.js" id="vkm-cookie-banner-js"></script>',
                htmlspecialchars($this->assetsUrl)
            );
        } else {
