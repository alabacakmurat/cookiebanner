...
<head>
    ...
    <title>VKM Cookie Banner - Demo</title>
    <style>
        * {
        ...
        .event-item.consent-given { border-left-color: #22c55e; }
        .event-item.consent-updated { border-left-color: #f59e0b; }
        .event-item.consent-withdrawn { border-left-color: #ef4444; }
...
?&gt;</pre>
    ...
    <footer>
        <p>VKM Cookie Banner v1.0.0 | <a href="https://github.com/vkmadmins/cookiebanner">GitHub</a></p>
    </footer>
    ...
    <script>
        ...
        function updateConsentStatus() {
            ...
            if (window.vkmCookieBanner) {
                const consent = window.vkmCookieBanner.getConsentData();
        ...
        document.addEventListener('vkm:init', function(e) {
            updateConsentStatus();
            logEvent('vkm:init', { hasConsent: e.detail.consent !== null });
        });
        ...
        const events = [
            'vkm:consent:given',
            'vkm:consent:updated',
            'vkm:consent:withdrawn',
            'vkm:banner:shown',
            'vkm:banner:hidden',
            'vkm:preferences:opened',
            'vkm:preferences:closed',
            'vkm:script:loaded',
            'vkm:script:blocked',
            'vkm:category:enabled',
            'vkm:category:disabled'
        ];
        ...
        function showBanner() {
            if (window.vkmCookieBanner) {
                window.vkmCookieBanner.showBanner();
            }
        }

        function showPreferences() {
            if (window.vkmCookieBanner) {
                window.vkmCookieBanner.showPreferences();
            }
        }

        function acceptAllCookies() {
            if (window.vkmCookieBanner) {
                window.vkmCookieBanner.acceptAll();
            }
        }

        function rejectAllCookies() {
            if (window.vkmCookieBanner) {
                window.vkmCookieBanner.rejectAll();
            }
        }

        function withdrawConsent() {
            if (window.vkmCookieBanner) {
                window.vkmCookieBanner.withdrawConsent();
            }
        }
        ...
        setTimeout(function() {
            if (window.vkmCookieBanner && !window.vkmCookieBanner.hasConsent()) {
                window.vkmCookieBanner.showBanner();
            }
        }, 500);
    </script>
